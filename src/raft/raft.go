...
func (rf *Raft) BeFollower() {
	...
	for {
		...
		case <-rf.heartbeatChan:
			//rf.mu.Lock()
			//DPrintf(StateFine, "Term %v: Reset timeout for %v", rf.currentTerm, rf.me)
			//rf.mu.Unlock()
			continue
...
func (rf *Raft) BeCandidate() {
	...
	for peer := range rf.peers {
		...
		go func(peer int) { //send out vote requests in parallel
			...
			DPrintf(Count, "Term %v: %v sent request vote to %v", rf.currentTerm, rf.me, peer)
			rf.mu.Unlock()
			reply := RequestVoteReply{}
			ok := rf.peers[peer].Call("Raft.RequestVote", &request, &reply)
			rf.mu.Lock()
			defer rf.mu.Unlock()
			DPrintf(State, "Term %v: %v received response for vote from %v", rf.currentTerm, rf.me, peer)
			if !ok {
				return
			}
			if rf.role != Candidate {
